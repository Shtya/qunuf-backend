...
import { BulkCreatePropertyDto, BulkPropertyItemDto } from './dto/bulk-create-property.dto';
...
export class PropertiesService {
    ...
    ): Promise<{ created: Property[]; failed: { index: number; name: string; error: string }[] }> {
        const created: Property[] = [];
        const failed: { index: number; name: string; error: string }[] = [];

        for (let i = 0; i < dto.properties.length; i++) {
            const item = dto.properties[i];
            try {
                const slug = generateSlugHelper(item.name);
                const existing = await this.propertyRepo.findOne({ where: { slug } });
                if (existing) {
                    failed.push({ index: i, name: item.name, error: `Property with name "${item.name}" already exists` });
                    continue;
                }

                const stateExists = await this.stateRepo.findOne({ where: { id: item.stateId } });
                if (!stateExists) {
                    failed.push({ index: i, name: item.name, error: 'Selected state not found' });
                    continue;
                }
                ...
                const property = this.propertyRepo.create({
                    ...
                    userId,
                    status: PropertyStatus.PENDING,
                    ownershipType: item.ownershipType ?? OwnershipType.OWNER,
                    securityDeposit: item.securityDeposit ?? 0,
                    capacity: item.capacity,
                    isFurnished: false,
                    propertyNumber: item.propertyNumber || 'N/A',
                    nationalAddressCode: item.nationalAddressCode || '00000000',
                ...
                failed.push({ index: i, name: item.name, error: err?.message ?? 'Unknown error' });
            }
        }
    ...
    async findAll(user: any, query: PropertyFilterDto) {
        ...
        if (query.buildingType) {
            filters.buildingType = query.buildingType;
        }
        if (query.propertyPurpose) {
            filters.propertyPurpose = query.propertyPurpose;
        }
        if (query.paymentCycle) {
            filters.paymentCycle = query.paymentCycle;
        }
    ...
    async getRecentProperties(user: any, startDate?: Date, endDate?: Date) {
        const now = new Date();
        ...
        const filters: Record<string, any> = { created_at: Between(start, end) };
