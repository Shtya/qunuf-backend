...
import { BuildingType, CommercialSubType, DocumentType, FurnishingStatus, OwnershipType, PaymentCycle, PropertyPurpose, PropertyType, PropertyUsage, RentType, ResidentialSubType } from 'src/common/entities/property.entity';
import { IsPastDate, IsYearRange } from 'src/common/utils/validators';


class FacilityDto {
    @IsString() @MaxLength(100) name: string;
    @IsNumber() @Type(() => Number) @Min(0) @Max(50) distance_km: number;
}
class PropertyFacilitiesDto {
    @IsOptional() @IsNumber() @Type(() => Number) @Min(0) livingRooms?: number;
    @IsOptional() @IsNumber() @Type(() => Number) @Min(0) parking?: number;
    @IsOptional() @IsNumber() @Type(() => Number) @Min(0) elevators?: number;
    @IsOptional() @IsNumber() @Type(() => Number) @Min(0) bathrooms?: number;
    @IsOptional() @IsNumber() @Type(() => Number) @Min(0) securityEntrances?: number;
    @IsOptional() @IsNumber() @Type(() => Number) @Min(0) bedrooms?: number;
    @IsOptional() @IsBoolean() @Type(() => Boolean) maidRoom?: boolean;
    @IsOptional() @IsNumber() @Type(() => Number) @Min(0) kitchen?: number;
    @IsOptional() @IsNumber() @Type(() => Number) @Min(0) store?: number;
    ...
    @IsOptional() @IsNumber() @Type(() => Number) @Min(0) majlis?: number;
    @IsOptional() @IsNumber() @Type(() => Number) @Min(0) rooms?: number;
}
...
export class CreatePropertyDto {
    ...
    @IsOptional() @IsNumber() @Type(() => Number) @IsNotEmpty() @Min(1) capacity?: number;
    ...
    @IsString() @IsNotEmpty() @MinLength(3) @MaxLength(250) issuedBy: string;//
    ...
    // Document location
    @IsOptional() @IsString() @MaxLength(200) documentIssueLocation?: string;

    // Building details
    @IsOptional() @IsEnum(BuildingType) buildingType?: BuildingType;
    @IsOptional() @IsEnum(PropertyPurpose) propertyPurpose?: PropertyPurpose;
    @IsOptional() @IsEnum(PropertyUsage) propertyUsage?: PropertyUsage;
    @IsOptional() @IsNumber() @Type(() => Number) @Min(0) @Max(200) numberOfFloors?: number;
    @IsOptional() @IsNumber() @Type(() => Number) @Min(0) @Max(10000) numberOfUnits?: number;
    @IsOptional() @IsNumber() @Type(() => Number) @Min(0) @Max(10000) numberOfShops?: number;

    // Unit details
    @IsOptional() @IsString() @MaxLength(50) unitNumber?: string;
    @IsOptional() @IsNumber() @Type(() => Number) @Min(-10) @Max(500) floorNumber?: number;
    @IsOptional() @IsBoolean() @Type(() => Boolean) hasCombinedAcUnit?: boolean;
    @IsOptional() @IsNumber() @Type(() => Number) @Min(0) @Max(100) acUnitsCount?: number;
    @IsOptional() @IsEnum(FurnishingStatus) furnishingStatus?: FurnishingStatus;
    @IsOptional() @IsBoolean() @Type(() => Boolean) hasKitchenCabinets?: boolean;

    // Sublease (Section 9)
    @IsOptional() @IsBoolean() @Type(() => Boolean) subleaseAllowed?: boolean;

    // Meter readings
    @IsOptional() @IsNumber() @Type(() => Number) @Min(0) electricityMeterReading?: number;
    @IsOptional() @IsNumber() @Type(() => Number) @Min(0) gasMeterReading?: number;
    @IsOptional() @IsNumber() @Type(() => Number) @Min(0) waterMeterReading?: number;

    // Financial extras (Section 10)
    @IsOptional() @IsNumber() @Type(() => Number) @Min(0) brokerageFee?: number;
    @IsOptional() @IsNumber() @Type(() => Number) @Min(0) electricityMonthlyAmount?: number;
    @IsOptional() @IsNumber() @Type(() => Number) @Min(0) gasMonthlyAmount?: number;
    @IsOptional() @IsNumber() @Type(() => Number) @Min(0) waterMonthlyAmount?: number;
    @IsOptional() @IsNumber() @Type(() => Number) @Min(0) parkingMonthlyAmount?: number;
    @IsOptional() @IsNumber() @Type(() => Number) @Min(0) @Max(1000) parkingLotsRented?: number;

    // Contract defaults
    @IsOptional() @IsEnum(PaymentCycle) paymentCycle?: PaymentCycle;
    @IsOptional() @IsString() @MaxLength(3000) article11?: string;
