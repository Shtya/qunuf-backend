...
import { Between, DataSource, In, LessThan, LessThanOrEqual, Repository } from 'typeorm';
import { CreateContractDto } from './dto/create-contract.dto';
import { AdminCreateContractDto } from './dto/admin-create-contract.dto';
...
export class ContractsService {
  ...
  async create(tenantId: string, dto: CreateContractDto, skipProfileValidation = false) {
    ...
    if (!skipProfileValidation) this.validateTenantProfile(tenant);
    ...
    if (
      ...
      const enteredYears = durationInYears.toFixed(2);
      throw new BadRequestException(
        `This is a Residential property — the maximum allowed contract duration is ${this.MAX_RENT_YEAR_RESIDENTIAL} year (12 months). ` +
        `You entered ${property.rentType === RentType.MONTHLY ? `${duration} months (≈ ${enteredYears} years)` : `${duration} years`}. ` +
        `Please reduce the duration to 12 months or less.`
      );
    }
    ...
    if (
      ...
      const enteredYears = durationInYears.toFixed(2);
      throw new BadRequestException(
        `This is a Commercial property — the maximum allowed contract duration is ${this.MAX_RENT_YEAR_COMMERCIAL} years. ` +
        `You entered ${property.rentType === RentType.MONTHLY ? `${duration} months (≈ ${enteredYears} years)` : `${duration} years`}. ` +
        `Please reduce the duration to ${this.MAX_RENT_YEAR_COMMERCIAL * 12} months or less.`
      );
    }
  ...
  async adminCreate(dto: AdminCreateContractDto) {
    const { tenantId, directActivate, ...contractDto } = dto;
    const contract = await this.create(tenantId, contractDto, true);

    if (directActivate && contract?.id) {
  ...
  async getDashboardStats(user: any, startDate?: Date, endDate?: Date) {
    const now = new Date();
    const end = endDate ?? new Date(now.getFullYear(), now.getMonth() + 1, 0, 23, 59, 59, 999);
    const start = startDate ?? new Date(now.getFullYear(), now.getMonth(), 1, 0, 0, 0, 0);
    ...
    if (user.role === UserRole.ADMIN) {
      const [
        ...
        freeNow, freeBefore,
        rentedNow, rentedBefore,
      ] = await Promise.all([
        this.propertyRepo.count({ where: { created_at: Between(start, end) } }),
        this.propertyRepo.count({ where: { created_at: Between(prevStart, prevEnd) } }),
        this.propertyRepo.count({ where: { isRented: false, created_at: Between(start, end) } }),
        this.propertyRepo.count({ where: { isRented: false, created_at: Between(prevStart, prevEnd) } }),
        this.propertyRepo.count({ where: { isRented: true, created_at: Between(start, end) } }),
        this.propertyRepo.count({ where: { isRented: true, created_at: Between(prevStart, prevEnd) } }),
      ]);

      return {
        totalProperties: { value: totalNow, changePercent: this.percentChange(totalNow, totalBefore) },
        freeProperties: { value: freeNow, changePercent: this.percentChange(freeNow, freeBefore) },
        rentedProperties: { value: rentedNow, changePercent: this.percentChange(rentedNow, rentedBefore) },
      };
    }
    ...
    if (user.role === UserRole.LANDLORD) {
      const [
        ...
        freeNow, freeBefore,
        rentedNow, rentedBefore,
        reviewsNow, reviewsBefore,
      ] = await Promise.all([
        this.propertyRepo.count({ where: { userId: user.id, created_at: Between(start, end) } }),
        this.propertyRepo.count({ where: { userId: user.id, created_at: Between(prevStart, prevEnd) } }),
        this.propertyRepo.count({ where: { userId: user.id, isRented: false, created_at: Between(start, end) } }),
        this.propertyRepo.count({ where: { userId: user.id, isRented: false, created_at: Between(prevStart, prevEnd) } }),
        this.propertyRepo.count({ where: { userId: user.id, isRented: true, created_at: Between(start, end) } }),
        this.propertyRepo.count({ where: { userId: user.id, isRented: true, created_at: Between(prevStart, prevEnd) } }),
        this.reviewRepo.count({ where: { created_at: Between(start, end) } }),
        this.reviewRepo.count({ where: { created_at: Between(prevStart, prevEnd) } }),
      ]);
      ...
      return {
        ...
        freeProperties: { value: freeNow, changePercent: this.percentChange(freeNow, freeBefore) },
        rentedProperties: { value: rentedNow, changePercent: this.percentChange(rentedNow, rentedBefore) },
        totalReviews: { value: reviewsNow, changePercent: this.percentChange(reviewsNow, reviewsBefore) },
      };
    }
    ...
    if (user.role === UserRole.TENANT) {
      const [
        ...
        ejarNowRaw, ejarBeforeRaw,
      ] = await Promise.all([
        this.contractRepo.count({ where: { tenantId: user.id, status: ContractStatus.ACTIVE, created_at: Between(start, end) } }),
        this.contractRepo.count({ where: { tenantId: user.id, status: ContractStatus.ACTIVE, created_at: Between(prevStart, prevEnd) } }),
        this.renewRepo.count({ where: { tenantId: user.id, status: RenewStatus.PENDING, created_at: Between(start, end) } }),
        this.renewRepo.count({ where: { tenantId: user.id, status: RenewStatus.PENDING, created_at: Between(prevStart, prevEnd) } }),
        this.contractRepo
        ...
        this.contractRepo
          ...
          .where('c.tenantId = :id', { id: user.id })
          .andWhere('c.ejarPdfPath IS NOT NULL')
          .andWhere('c.created_at BETWEEN :prevStart AND :prevEnd', { prevStart, prevEnd })
          .getRawOne(),
      ]);

      return {
        activeContracts: { value: activeNow, changePercent: this.percentChange(activeNow, activeBefore) },
        totalPendingRenewRequests: { value: pendingNow, changePercent: this.percentChange(pendingNow, pendingBefore) },
        totalAmountWithEjar: { value: Number(ejarNowRaw.sum), changePercent: this.percentChange(Number(ejarNowRaw.sum), Number(ejarBeforeRaw.sum)) },
      };
    }
  ...
  async getDashboardChartData(user: any, startDate?: Date, endDate?: Date) {
    const now = new Date();
    const end = endDate ?? new Date(now.getFullYear(), now.getMonth() + 1, 0, 23, 59, 59, 999);
    const start = startDate ?? new Date(now.getFullYear(), now.getMonth(), 1, 0, 0, 0, 0);
    ...
    if (user.role === UserRole.ADMIN) {
      ...
      // Fill every day in the range with 0, then overlay actual counts
      ...
      return {
        contractsPerDay: Array.from(dateMap.values()),
    ...
    if (user.role === UserRole.LANDLORD) {
      const contracts = await this.contractRepo.find({
        where: { landlordId: user.id, created_at: Between(start, end) },
      ...
      return { rentedAnalytics: monthlyData };
    }

    return {};
  }

  async getRecentContracts(user: any, startDate?: Date, endDate?: Date) {
    const now = new Date();
    const end = endDate ?? new Date(now.getFullYear(), now.getMonth() + 1, 0, 23, 59, 59, 999);
    const start = startDate ?? new Date(now.getFullYear(), now.getMonth(), 1, 0, 0, 0, 0);
    ...
    const query = this.contractRepo
      ...
      .where('contract.created_at BETWEEN :start AND :end', { start, end })
      .orderBy('contract.created_at', 'DESC')
      .take(7);

    // Apply Role Filters
    if (user.role === UserRole.LANDLORD) {
      query.andWhere('contract.landlordId = :userId', { userId: user.id });
    } else if (user.role === UserRole.TENANT) {
      query.andWhere('contract.tenantId = :userId', { userId: user.id });
    }
