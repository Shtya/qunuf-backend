...
export class ContractsController {
  ...
  @Post('admin')
  @Auth(UserRole.ADMIN)
  @ApiOperation({ summary: 'Admin creates a contract on behalf of a tenant' })
  @ApiResponse({ status: 201, description: 'Contract created successfully by admin.' })
  adminCreate(@Body() dto: AdminCreateContractDto) {
    return this.contractsService.adminCreate(dto);
  }
  ...
  async getDashboardChartData(
    ...
    return this.contractsService.getDashboardChartData(
      user,
      startDate ? new Date(startDate) : undefined,
      endDate ? new Date(endDate) : undefined,
    );
  ...
  async getRecentContracts(
    ...
    return this.contractsService.getRecentContracts(
      user,
      startDate ? new Date(startDate) : undefined,
      endDate ? new Date(endDate) : undefined,
    );
