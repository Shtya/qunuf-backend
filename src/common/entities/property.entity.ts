...
export class Property extends CoreEntity {
    ...
    @Column({ type: 'decimal', precision: 12, scale: 3, nullable: true, name: 'electricity_meter_reading' })
    electricityMeterReading: number;

    @Column({ type: 'decimal', precision: 12, scale: 3, nullable: true, name: 'gas_meter_reading' })
    gasMeterReading: number;

    @Column({ type: 'decimal', precision: 12, scale: 3, nullable: true, name: 'water_meter_reading' })
    waterMeterReading: number;

    // Document location
    @Column({ nullable: true, name: 'document_issue_location' })
    documentIssueLocation: string;

    // Building details
    @Column({ type: 'enum', enum: BuildingType, nullable: true, name: 'building_type' })
    buildingType: BuildingType;

    @Column({ type: 'enum', enum: PropertyPurpose, nullable: true, name: 'property_purpose' })
    propertyPurpose: PropertyPurpose;

    @Column({ type: 'enum', enum: PropertyUsage, nullable: true, name: 'property_usage' })
    propertyUsage: PropertyUsage;

    @Column({ type: 'int', nullable: true, name: 'number_of_floors' })
    numberOfFloors: number;

    @Column({ type: 'int', nullable: true, name: 'number_of_units' })
    numberOfUnits: number;

    @Column({ type: 'int', nullable: true, name: 'number_of_shops' })
    ...
    @Column({ type: 'boolean', nullable: true, name: 'has_combined_ac_unit' })
    hasCombinedAcUnit: boolean;

    @Column({ type: 'int', nullable: true, name: 'ac_units_count' })
    acUnitsCount: number;

    @Column({ type: 'enum', enum: FurnishingStatus, nullable: true, name: 'furnishing_status' })
    furnishingStatus: FurnishingStatus;

    @Column({ type: 'boolean', nullable: true, name: 'has_kitchen_cabinets' })
    hasKitchenCabinets: boolean;

    // Sublease
    @Column({ type: 'boolean', default: false, name: 'sublease_allowed' })
    subleaseAllowed: boolean;

    // Financial (from contract Section 10)
    @Column({ type: 'decimal', precision: 10, scale: 2, nullable: true, name: 'brokerage_fee' })
    brokerageFee: number;

    @Column({ type: 'decimal', precision: 10, scale: 2, nullable: true, name: 'electricity_monthly_amount' })
    electricityMonthlyAmount: number;

    @Column({ type: 'decimal', precision: 10, scale: 2, nullable: true, name: 'gas_monthly_amount' })
    gasMonthlyAmount: number;

    @Column({ type: 'decimal', precision: 10, scale: 2, nullable: true, name: 'water_monthly_amount' })
    waterMonthlyAmount: number;

    @Column({ type: 'decimal', precision: 10, scale: 2, nullable: true, name: 'parking_monthly_amount' })
    parkingMonthlyAmount: number;

    @Column({ type: 'int', nullable: true, name: 'parking_lots_rented' })
    parkingLotsRented: number;

    // Contract defaults
    @Column({ type: 'enum', enum: PaymentCycle, nullable: true, name: 'payment_cycle' })
    paymentCycle: PaymentCycle;

    @Column({ type: 'text', nullable: true, name: 'article11' })
    article11: string;

    @Column({ type: 'text', nullable: true, name: 'additional_terms' })
